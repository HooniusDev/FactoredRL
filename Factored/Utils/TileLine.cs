...
namespace HumbleRL.Utils
{
	class TileLine
	{
		...
		static public List<Point> GetLosLine( IMap Map, Point start, Point end, int sightRange, bool lightEndTile = true )
		{
			...
			for ( int step = 0; step <= N; step++ )
			{
				...
				if ( !Map.IsValid( p.X, p.Y ) || count == sightRange ) // No tile ie. void besides corridor
					break;
				points.Add( p );
				if ( !Map.IsTransparent(p) ) // blocks light but light tile if lightendtile 
				{
					if ( !lightEndTile )
						points.Remove( p );
					break;
